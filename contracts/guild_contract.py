...
class GuildContract(BaseFarmContract, BaseBoostedContract):
    ...
    def load_config_dict(cls, config_dict: dict):
        return GuildContract(farming_token=config_dict['farmingToken'],
    ...
    @classmethod
    def load_contract_by_address(cls, address: str, version=FarmContractVersion.V2Boosted):
        ...
        return GuildContract(farming_token, farm_token, farmed_token, address, version)
    ...
    def stakeFarm(self, network_provider: NetworkProviders, user: Account, event: EnterFarmEvent, initial: bool = False) -> str:
        # TODO: remove initial parameter by using the event data
        function_purpose = "enter farm"
        logger.info(function_purpose)
        logger.debug(f"Account: {user.address}")

        stakeFarmFn = "stakeFarm"

        logger.info(f"Calling {stakeFarmFn} endpoint...")
        ...
        return multi_esdt_endpoint_call(function_purpose, network_provider.proxy, gas_limit, user,
                                        Address(self.address), stakeFarmFn, sc_args)

    def exitFarm(self, proxy: ProxyNetworkProvider, user: Account, event: ExitFarmEvent) -> str:
        function_purpose = f"unstake farm"
        logger.info(function_purpose)
        logger.debug(f"Account: {user.address}")
        ...
        return multi_esdt_endpoint_call(function_purpose, proxy, gas_limit, user,
                                        Address(self.address), "unstakeFarm", sc_args)
    
    def unbondFarm(self, proxy: ProxyNetworkProvider, user: Account, event: ExitFarmEvent) -> str:
        function_purpose = f"unbond farm"
        logger.info(function_purpose)
        logger.debug(f"Account: {user.address}")
        ...
        return multi_esdt_endpoint_call(function_purpose, proxy, gas_limit, user,
                                        Address(self.address), "unbondFarm", sc_args)
    
    def cancelUnbondFarm(self, proxy: ProxyNetworkProvider, user: Account, event: ExitFarmEvent) -> str:
        function_purpose = f"cancel unbond farm"
        logger.info(function_purpose)
        logger.debug(f"Account: {user.address}")
        ...
        return multi_esdt_endpoint_call(function_purpose, proxy, gas_limit, user,
                                        Address(self.address), "cancelUnbondFarm", sc_args)
    ...
    def calculateRewardsForUser(self, network_provider: NetworkProviders, user: Account, amount: int, attr: list):
        ...
        return endpoint_call(network_provider.proxy, gas_limit, user, Address(self.address), fn, sc_args)
    
    def mergeTokens(self, user: Account, proxy: ProxyNetworkProvider, args: list = []):
        """Expecting as args:
            type[List[ESDTTokens]]: tokens to merge
            type[str]: farm address
        """
        function_purpose = "merge farm tokens"
        logger.info(function_purpose)
        ...
        gas_limit = 50000000
        return multi_esdt_endpoint_call(function_purpose, proxy, gas_limit, user, Address(self.address), "mergeFarmTokens", args)
    ...
    def get_all_stats(self, proxy: ProxyNetworkProvider) -> Dict[str, Any]:
        all_stats = {}
