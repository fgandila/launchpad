    ...
    for index in range(0, args.num_repeat):
        ...
        for contract in contracts:
            ...
            for instance_index in range(0, num_instances):
                ...
                bytecode: str = load_code_as_hex(bin_file)
                ...
                tokens_per_win_ticket = str(lp_config.TOKENS_PER_WINNING_TICKET)
                ticket_price = str(lp_config.TICKET_PRICE)
                winning_tickets = str(lp_config.NR_WINNING_TICKETS)
                ...
                if contract.get_name() == "launchpad-v2":
                    arguments.extend([nft_payment_token_hex, nft_payment_token_nonce, nft_cost, winning_nfts, max_tier])
                if contract.get_name() == "launchpad-v3":
                    arguments.extend([max_tier, locked_tokens_percentage, locked_tokens_unlock_epoch, locking_address])
                if contract.get_name() == "launchpad-v4":
                    arguments.extend([max_tier])

                config.CONTRACTS[contract.get_name()]["deploy-arguments"] = arguments

                deploy_transaction, contract_address = contract.get_deploy_transaction(bytecode, deployer, network)
        ...
        broadcast_transactions(transactions, proxy, chunk_size=args.chunk_size, sleep=args.sleep_between_chunks,
