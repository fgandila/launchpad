...
from multiversx_sdk import ProxyNetworkProvider, Transaction

from arrows.stress.contracts import config
from arrows.stress.contracts.contract import FlowConfig, NetworkConfig
...
from arrows.stress.shared import (BunchOfAccounts, broadcast_transactions_new,
...
def main(cli_args: List[str]):
    ...
    proxy_old = ElrondProxy(args.proxy)
    ...
    for index in range(0, args.num_repeat):
        ...
        bunch_of_accounts.sync_nonces(proxy_old)
        flow_control.sync_contract_owner_nonces(proxy_old)
        ...
        for tracks_file in tracks_files:
            ...
            hashes = broadcast_transactions_new(transactions, proxy, chunk_size=args.chunk_size, sleep=args.sleep_between_chunks, confirm_yes=args.yes)
            print(hashes)
